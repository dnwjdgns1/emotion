#!/usr/bin/env python
# coding: utf-8
import os  #os함수를 모듈(파일을 복사하거나 디렉터리를 생성하고 특정 디렉터리 내의 파일 목록을 구할 때)
import torch #gpu를 사용하여 수치연산을 가속
...
# %%
random_seed = 1234 
torch.manual_seed(random_seed) #똑같은 세팅으로 실험을 돌렸음에도 결과가 다르게 나오는 것을 막기 위해 파이토치 자체의 난수 생성기도 시드를 설정해주는 것
...
def get_dataloader(args):
    classes_train = os.listdir(args.data_path + "\\train")#train데이터 경로 안의 배열 값을 정리
    classes_valid = os.listdir(args.data_path + "\\val")#val데이터 경로 안의 배열 값을 정리
    print("Data Folders -", os.listdir(args.data_path))#안내메세지
    print("Train Folders -", classes_train)#안내메세지
    print("Validation Folders -", classes_valid)#안내메세지
    ...
    ])#트래인을 변환 이미지 크기를 사진의 크기에 맞게, 그레이스케일 색으로 이미지를 좌우반전하고 pythorch tensor타입으로 변경 랜덤으로 회전 색깔바꾸기 이미지 한번에 합치기
    ...
    valid_transforms = tt.Compose([
        ...
        tt.ToTensor(),
        #tt.Normalize((0.5), (0.5), inplace=True)
    ])#이건 답지기 때문에 많이 변경할 필요는 없지만 train은 아니다 # 학습이 이미 완료된 모델을 검증하기 위한 데이터셋

    # Datasets from ImageFolder
    train_dataset = ImageFolder(args.data_path + '\\train', train_transforms) #데이터 셋은 여기 안에 있다
    valid_dataset = ImageFolder(args.data_path + '\\val', valid_transforms)
...
def train_epoch(model, dataset_loader, epoch, device, optimizer, criterion):
    ...
    for i, (data, target) in enumerate(dataset_loader): #내장된 함수로 무한루프 돌리기
...
def evaluation(model, dataset_loader, device, criterion):
    ...
    accuracy = 100 * correct / len(dataset_loader.dataset)   #정확도 구하기
...
def getOptimizer(args, model): #객체 변수 최적화하기
...
def main():
    ...
    parser.add_argument('--data_path', action='store',
                        default='C:/Users/etri_wi/Desktop/zzzz/data', help='path of data folder')
    parser.add_argument('--model_path', action='store',
                        default='model_eff.pth', help='path of model file')
    parser.add_argument('--model', action='store',
                        default='efficientnet-b5', help='network architecture')
    ...
    parser.add_argument('--batch_size', action='store', type=int,
                        default=128, help='size of batch')
