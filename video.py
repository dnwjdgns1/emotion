 #!/usr/bin/env python
# coding: utf-8
from PIL import Image, ImageFont, ImageDraw #pil함수중에서 저 3개만 사용하겠다
import os 
import argparse #인자값을 줘서 동작을 다르게 하고 싶을 때 사용하는 경우
import cv2 #opencv파이썬
import numpy as np 
import torch
...
face_classifier = cv2.CascadeClassifier('face_classifier.xml')# 객체 생성 및 함수 불러오기

display_color = (246, 189, 86) #r,g,b컬러

def softmax(x):
    e_x = np.exp(x - np.max(x))
    return e_x / e_x.sum() # 하나의 값에 의해 여러개의 클래스에 분류될 확률이 각각 출력 다중클래스 분류 모델을 만들때 사용하고 결과를 확률로 해석할수 있게 변환해주는 함수로
...
def main(args):
    ...
    if args.gpu and torch.cuda.is_available():
        ...
        device = 'cpu'
        #print('GPU Off')   #gpu와 쿠다가 같이 사용가능하면 쿠다 사용 아니면 cpu사용

    model_state = torch.load(args.model_path, map_location=torch.device(device)) #모델 경로를 저장하고
